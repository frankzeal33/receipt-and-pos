...
import { addCustomerInput, correctSaleInput, deleteCustomerInput, editCustomerInput, makeSaleInput, saleByIdInput } from "../types/zodtypes/saleType.ts";
import asyncHandler from "express-async-handler";
import { cleanInput } from "../utils/helpers.ts";
import { paginationInput } from "../types/zodtypes/staffType.ts";


export const makeSale = asyncHandler(async (req: Request, res: Response) => {
  
  const { items, discount = 0, taxRate = 0, paymentType, customerName, customerId, customerIdentifier } = (req.validated as makeSaleInput).body;
  ...
  const sale = await prisma.$transaction(async (tx) => {
    ...
    }) as Prisma.SaleGetPayload<{ include: { saleItems: true } }>;
...
export const correctSale = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { items, discount = 0, taxRate = 0, paymentType, customerName, customerId, customerIdentifier } = (req.validated as correctSaleInput).body;
  const { wrongSaleId } = (req.validated as correctSaleInput).params;
...
export const refundSale = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { saleId } = (req.validated as saleByIdInput).params;
...
export const addCustomer = asyncHandler(async (req: Request, res: Response) => {
  const { name, email, phone, address } = (req.validated as addCustomerInput).body;
  const companyId = req.user.companyId;
...
export const editCustomer = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { customerId  } = (req.validated as editCustomerInput).params;
  const { name, email, phone, address } = (req.validated as editCustomerInput).body;
...
export const deleteCustomer = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { customerId  } = (req.validated as deleteCustomerInput).params;
...
export const getSales = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { search, page, limit, status } = (req.validated as paginationInput).query;
...
export const getSaleById = asyncHandler(async (req: Request, res: Response) => {
  ...
  const totalAmount = sale.saleItems.reduce(
    (acc, item) => acc + item.quantity * item.price.toNumber(),
    0
  );
...
export const getNotifications = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { page, limit } = (req.validated as paginationInput).query;
...
export const getIsRead = asyncHandler(async (req: Request, res: Response) => {
...
export const getSalesChart = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const result: Record<string, { date: string; paid: number; refunded: number; corrected: number }> = {};
  ...
  const chartData = Object.values(result).sort(
