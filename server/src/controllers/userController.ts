...
import { forgotPasswordInput, loginInput, registerInput, resendRegVerificationInput, resetPasswordInput, updateInput, verifyRegInput, verifyRegLinkInput } from "../types/zodtypes/authType.ts";
import generateToken from "../utils/generateToken.ts";
import bcrypt from "bcryptjs";
import { sendEmail } from "../utils/emailTransporter.ts";
import { renderTemplate } from "../utils/renderTemplate.ts";
import { delCache, getCache, setCache } from "../utils/cacheInstance.ts";
import logger from "../utils/logger.ts";
import { maskIDLog } from "../utils/masks.ts";
import { generateFrontendToken, generateOtp } from "../utils/randomTokens.ts";
...
const registerUser = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    const { firstName, lastName, email, password } = (req.validated as registerInput).body;
 ...
 */
const verifyRegOtp = asyncHandler(async (req: Request, res: Response): Promise<void> => {

    const { email, otp } = (req.validated as verifyRegInput).body;

    const userEmail = email.toLowerCase();

    const cachedOtp = await getCache<string>(`otp_${userEmail}`);
 ...
 *         description: User not found
 */
const verifyRegLink = asyncHandler(async (req: Request, res: Response): Promise<void> => {

    const { token } = (req.validated as verifyRegLinkInput).query;

    const cached = await getCache(`verify_${token}`) as string | null;;
...
const resendRegVerificationEmail = asyncHandler(async (req: Request, res: Response): Promise<void> => {
     
    const { email } = (req.validated as resendRegVerificationInput).body;
...
const forgotPassword = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const { email } = (req.validated as forgotPasswordInput).body;
...
const verifyForgotPasswordOtp = asyncHandler(async (req: Request, res: Response): Promise<void> => {
...
const verifyForgotPasswordLink = asyncHandler(async (req: Request, res: Response): Promise<void> => {
 
  const { token } = (req.validated as verifyRegLinkInput).query;

  // Lookup token in cache
  const cachedData = await getCache(`verify_${token}`) as string | null;
...
const resetPassword = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const { resetToken, newPassword } = (req.validated as resetPasswordInput).body;

  // Lookup reset token in cache
  const email = await getCache<string>(`reset_${resetToken}`);
...
const loginUser = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const { email, password, rememberMe} = (req.validated as loginInput).body;
...
const updateUserProfile = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const { firstName, lastName } = (req.validated as updateInput).body;
