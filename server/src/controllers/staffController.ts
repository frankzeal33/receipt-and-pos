...
import { AllRole, ExpenseCategory, Prisma } from "@prisma/client";
import { ManagerPermissions } from "../utils/permissions/Manager.ts";

export const addStaff = expressAsyncHandler(async (req: Request, res: Response): Promise<void> => {

    const { firstName, lastName, email, role, password, branchId } = (req.validated as addStaffInput).body;
    ...
    if (req.user.role === AllRole.MANAGER) {
      ...
      const allowedRoles: AllRole[] = [AllRole.SALES_PERSON, AllRole.ACCOUNTANT];
    ...
    const topRoles: AllRole[] = [AllRole.CO_CEO, AllRole.GENERAL_MANAGER, AllRole.GENERAL_ACCOUNTANT];

    if (topRoles.includes(role as AllRole)) {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        const addNotification = await tx.notification.create({
            data: {
                title: `${req.user.email} (${req.user.role}${
                  req.user.branch ? " - " + req.user.branch.name : ""
                }) added a staff`,
                description: `A staff ${newStaff.email} with ${newStaff.role} role was added by a ${req.user.role} ${
                  newStaff.branch ? `in branch ${newStaff.branch.name}` : ""
                }`,
                type: "USER",
                companyId: req.user.companyId,
...
export const editStaff = expressAsyncHandler(async (req: Request, res: Response): Promise<void> => {

    const { staffId, firstName, lastName, role, status } = (req.validated as editStaffInput).body;
...
export const deleteStaff = expressAsyncHandler(async (req: Request, res: Response): Promise<void> => {

    const { staffId } = (req.validated as deleteStaffInput).params;
...
export const addProduct = expressAsyncHandler(async (req: Request, res: Response): Promise<void> => {

    const { productName, productDesc, category, brand, price, quantity, status, branchId } = (req.validated as addProductInput).body;
...
export const editProduct = expressAsyncHandler(async (req: Request, res: Response): Promise<void> => {

    const { productId, productName, productDesc, category, brand, price, quantity, status, branchId } = (req.validated as editProductInput).body;
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        const addNotification = await tx.notification.create({
          data: {
            ...
            description: `Updated Product info: #Name-${updateProduct.productName} #Price-${updateProduct.price} #Quantity-${updateProduct.quantity} #Status-${updateProduct.status} ${
               updateProduct.branch ? `in branch ${updateProduct.branch.name}` : ""
            }`,
            type: "PRODUCT",
            companyId: req.user.companyId,
            branchId: req.user.branchId || null,
...
export const deleteProduct = expressAsyncHandler(async (req: Request, res: Response): Promise<void> => {

    const { productId } = (req.validated as productIdInput).params;
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        const addNotification = await tx.notification.create({
            data: {
                ...
                description: `Product info: #Name-${deleteProduct.productName} #Price-${deleteProduct.price} #Quantity-${deleteProduct.quantity} #Status-${deleteProduct.status} ${
                  deleteProduct.branch ? `in branch ${deleteProduct.branch.name}` : ""
                }`,
                type: "PRODUCT",
                companyId: req.user.companyId,
                branchId: req.user.branchId || null,
...
export const addExpense = expressAsyncHandler(async (req: Request, res: Response) => {

  const { title, description, category, amount, paymentType, expenseDate } = (req.validated as addExpenseInput).body;
...
export const editExpense = expressAsyncHandler(async (req: Request, res: Response) => {

  const {  branchId, title, description, category, amount, paymentType, expenseDate } = (req.validated as editExpenseInput).body;
  const { expenseId } = (req.validated as editExpenseInput).params;
...
export const deleteExpense = expressAsyncHandler(async (req: Request, res: Response) => {

  const { branchId } = (req.validated as expenseIdInput).body;
  const { expenseId } = (req.validated as expenseIdInput).params;
  ...
  const result = await prisma.$transaction(async (tx) => {
        ...
        const addNotification = await tx.notification.create({
            data: {
                ...
                description: `Expense info: #Title-${deletedExpense.title} #Desc-${deletedExpense.description} #Category-${deletedExpense.category} #Amount-${deletedExpense.amount} #PaymentType-${deletedExpense.paymentType} #ExpenseDate-${deletedExpense.expenseDate} ${
                  deletedExpense.branch ? `in branch ${deletedExpense.branch.name}` : ""
                }`,
                type: "EXPENSE",
                companyId: req.user.companyId,
...
export const addInvoice = expressAsyncHandler(async (req: Request, res: Response) => {
  ...
  const { 
    ...
    note,
    branchId
  } = (req.validated as addInvoiceInput).body;
...
export const editInvoice = expressAsyncHandler(async (req: Request, res: Response) => {
  ...
  const { 
    ...
    note,
    branchId
  } = (req.validated as editInvoiceInput).body;
  const { invoiceId } = (req.validated as editInvoiceInput).params;
...
export const deleteInvoice = expressAsyncHandler(async (req: Request, res: Response) => {

  const { invoiceId } = (req.validated as deleteInvoiceInput).params;
  const { branchId } = (req.validated as deleteInvoiceInput).body;
...
export const getStaffs = expressAsyncHandler(async (req: Request, res: Response) => {

  const { search, role, page, limit } = (req.validated as paginationInput).query;
...
export const getExpenses = expressAsyncHandler(async (req: Request, res: Response) => {

  const { category, startDate, endDate, page, limit } = (req.validated as paginationCategoryInput).query;
...
export const getInvoices = expressAsyncHandler(async (req: Request, res: Response) => {
...
export const getInvoiceById = expressAsyncHandler(async (req: Request, res: Response) => {

  const { invoiceId } = (req.validated as getInvoiceByIdInput).params;
