...
import { CustomerType, NotificationType } from '@/types/General'
import CustomerMore from './_components/NotificationMore'
import NotificationMore from './_components/NotificationMore'
  ...
  const [count, setCount] = useState(0)
   const tableList = new Array(8).fill(null)
    ...
    const getNotifications = async () => {
        ...
        const response = await axiosClient.get(`/sales/get-notifications?page=${page}&page_size=${pageSize}`)
    ...
    <div className='my-container'>
        ...
        <div className='bg-light p-3 rounded-xl border w-full'>
            <div className='flex items-center gap-2 mb-4'>
                <p className="text-lg font-medium leading-none">Notifications ({count || 0})</p>
            </div>
                ...
                <div className="w-full min-h-[58vh] flex flex-col items-center justify-between overflow-x-auto">
                            ...
                            notifications.length !== 0 &&
                                ...
                                <TableBody>
                                    ...
                                    <TableRow key={notification?.id}>
                                        <TableCell className={`font-semibold`}>{ReduceTextLength(notification?.title, 70)}</TableCell>
                                        <TableCell className={`capitalize font-semibold`}>{ReduceTextLength(notification?.description, 50)}</TableCell>
                                        <TableCell className={`capitalize font-semibold`}>{formatEnums(notification?.role)}</TableCell>
                                        <TableCell className={`capitalize font-semibold`}>{format(new Date(notification?.createdAt), "dd MMM yyyy hh:mm a")}</TableCell>
                                        <TableCell className='capitalize bg-muted/30'>
                                            <NotificationMore notification={notification} getNotifications={getNotifications}/>
                                        </TableCell>
                                    </TableRow>
                                    ))}
                                </TableBody>
                    ...
                    {notifications.length === 0 &&
                        <div className='flex flex-col items-center justify-center min-h-[50vh] w-full'>
                            <NotFound imageStyle='size-14' title='No notifications found' desc='No notification have been added yet'/>
                        </div>
                    }
