...
import { getInvoiceType } from '@/types/General'
...
const page = () => {
  ...
  const [invoice, setInvoice] = useState<getInvoiceType | null>(null)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await axiosClient.post(`/staffs/edit-invoice/${id}`, invoiceApiData)
  ...
  useEffect(() => {
    if (invoice) {
      setForm({
        ...
        client_address: invoice?.customerAddress || "",
        client_phone: invoice?.customerPhone || "",
        note: invoice?.note || "",
        paymentTerm: invoice?.paymentTerm || "",
        extraChargeName: invoice?.extraChargeName || "",
        extraCharge: invoice?.extraCharge || "",
    ...
    if (invoice?.invoiceItems && Array.isArray(invoice.invoiceItems)) {
      const mappedItems = invoice.invoiceItems.map((item: any, index: number) => {
        ...
        // Calculate subtotal and total including tax
  ...
  const getInvoice = async () => {
      ...
      const response = await axiosClient.get(`/staffs/get-invoice/${id}`)
        ...
        <div className='grid gap-6'>
          <Card className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-6">
            ...
            <div className="grid gap-2">
              ...
              <CurrencyPicker
                defaultValue={
                  currencyOptions.find((opt) => opt.value === invoice?.currency) || currencyOptions[0]
                }
              ...
              <CardContent>
                <div className="grid gap-6">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select value={form.paymentTerm}  onValueChange={(value) =>
